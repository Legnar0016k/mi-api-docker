// server.js - API con registro inteligente (un registro por día + actualizaciones)
...
// ======================================================
// ENDPOINT PRINCIPAL - SOLO consulta historial
// ======================================================
...
// ======================================================
// ENDPOINT PARA OBTENER TASA ACTUAL (sin registrar)
// ======================================================
...
// ======================================================
// ENDPOINT PARA REGISTRAR/ACTUALIZAR TASA
// ======================================================
app.post('/api/tasas/registrar', async (req, res) => {
    try {
        const { usd, actualizar } = req.body;
        const fechaHoy = new Date().toISOString().split('T')[0];
        
        // Obtener euro aproximado
        const eurAprox = usd / 1.05;
        
        if (actualizar) {
            // ACTUALIZAR el registro de hoy
            await dbRun(`
                UPDATE tasas_historicas 
                SET usd = ?, eur = ?, fuente = 'actualizado', created_at = CURRENT_TIMESTAMP
                WHERE fecha = ?
            `, [usd, eurAprox, fechaHoy]);
            
            console.log(`🔄 Tasa actualizada: Bs ${usd.toFixed(2)} | ${fechaHoy}`);
            
            res.json({
                success: true,
                message: 'Tasa actualizada',
                data: { fecha: fechaHoy, usd, eur: eurAprox }
            });
        } else {
            // Verificar si ya existe
            const existe = await dbGet(`
                SELECT id FROM tasas_historicas WHERE fecha = ?
            `, [fechaHoy]);
            
            if (existe) {
                return res.json({
                    success: true,
                    message: 'Tasa ya registrada hoy',
                    data: { fecha: fechaHoy, usd }
                });
            }
            
            // INSERTAR nuevo registro
            await dbRun(`
                INSERT INTO tasas_historicas (fecha, usd, eur, fuente) 
                VALUES (?, ?, ?, 'registro')
            `, [fechaHoy, usd, eurAprox]);
            
            console.log(`✅ Tasa registrada: Bs ${usd.toFixed(2)} | ${fechaHoy}`);
...
// ======================================================
// ENDPOINT PARA OBTENER HISTORIAL SOLO
// ======================================================
...
async function agregarTasasIniciales() {
    const tasasIniciales = [
        ...
        { fecha: '2026-02-25', usd: 411.09, eur: 483.91 }
...
app.listen(PORT, () => {
    ...
    console.log(`   - GET  /api/tasas (historial completo)`);
    console.log(`   - GET  /api/tasas/actual (tasa actual)`);
    console.log(`   - POST /api/tasas/registrar (registrar/actualizar)`);
    console.log(`   - GET  /api/tasas/historial`);
});

//logica comentada a manera de respaldo 
